...
public class ArziHeaderComponent extends CustomComponent implements
		ClickListener {
	...
	public static Logger logger = LogManager
			.getLogger(ArziHeaderComponent.class);
	...
	private HorizontalLayout buildMainLayout() {
		...
		loggedInUserName.setStyleName("loggedInPtnt");
		mainLayout.addComponent(loggedInUserName);
		mainLayout.setComponentAlignment(loggedInUserName,
				Alignment.MIDDLE_RIGHT);
		mainLayout.setExpandRatio(loggedInUserName, 1.0f);
		...
		mainLayout.setExpandRatio(logout, 0.5f);
	...
	public void buttonClick(ClickEvent event) {
		if (event.getButton().equals(logout)) {
			...
			// "Logout" the patient
			logger.debug("Logging out the patient with ITS number: "
					+ (String) getSession().getAttribute(
							MedicalArziConstants.SESS_ATTR_PTNT_ITS_NUM)
					+ " and invalidating the session.");

			// Invalidate the underlying wrapped session (spring session) and invalidate it.
			...
			Page.getCurrent().setLocation("/map");
		}
	}
